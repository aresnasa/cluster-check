...
cleanup_old_reports() {
    ...
    for file in "$source_dir"/*_check_report_*.html; do
        if [[ -f "$file" ]]; then
            ...
            # 如果是第一次遇到这个主机，或者文件更新
            if [[ -z "${host_files[$host_key]}" ]]; then
                host_files[$host_key]="$file|$file_time"
            else
                local existing_file=$(echo "${host_files[$host_key]}" | cut -d'|' -f1)
                local existing_time=$(echo "${host_files[$host_key]}" | cut -d'|' -f2)
...
extract_status() {
    ...
    case "$check_type" in
        "system")
            # 系统检查状态 - 匹配HTML中的实际内容
            if grep -q "系统检查:.*success.*成功" "$report_file" 2>/dev/null; then
                echo "OK"
            elif grep -q "系统检查:.*error.*失败" "$report_file" 2>/dev/null; then
                echo "FAILED"
            elif grep -q "系统检查:.*warning.*警告" "$report_file" 2>/dev/null; then
                echo "WARNING"
            else
                echo "UNKNOWN"
            fi
        ...
        "kubernetes")
            # Kubernetes检查状态
            if grep -q "Kubernetes状态:.*success.*正常" "$report_file" 2>/dev/null; then
                echo "OK"
            elif grep -q "Kubernetes状态:.*error.*异常" "$report_file" 2>/dev/null; then
                echo "FAILED"
            elif grep -q "Kubernetes状态:.*warning.*警告" "$report_file" 2>/dev/null; then
                echo "WARNING"
        ...
        "gpu")
            # GPU/DCGM检查状态
            if grep -q "GPU状态:.*success.*正常\|DCGM状态:.*success.*正常" "$report_file" 2>/dev/null; then
                echo "OK"
            elif grep -q "GPU状态:.*error.*异常\|DCGM状态:.*error.*异常" "$report_file" 2>/dev/null; then
                echo "FAILED"
            elif grep -q "GPU状态:.*warning.*警告\|DCGM状态:.*warning.*警告" "$report_file" 2>/dev/null; then
                echo "WARNING"
            elif ! grep -q "GPU状态\|DCGM状态" "$report_file" 2>/dev/null; then
                echo "N/A"
            else
                echo "UNKNOWN"
            fi
        ...
        "resource")
            # 资源检查状态
            if grep -q "资源使用:.*success.*正常" "$report_file" 2>/dev/null; then
                echo "OK"
            elif grep -q "资源使用:.*warning.*警告" "$report_file" 2>/dev/null; then
                echo "WARNING"
            elif grep -q "资源使用:.*error.*异常" "$report_file" 2>/dev/null; then
                echo "FAILED"
            else
                echo "UNKNOWN"
            fi
